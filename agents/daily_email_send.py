...
def generate_llm_content(email_module, report: dict, config: dict, target_date: str, retries: int, retry_delay: float) -> tuple[dict | None, str | None]:
    return retry_optional(
        lambda: email_module.generate_email_content(report, config, target_date),
...
def build_failure_email_html(report: dict | None, config: dict, target_date: str, failure_reason: str) -> str:
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px; margin:0 auto;">
  ...
  <tr><td style="padding:16px 24px 0;"><p style="color:#374151; font-size:16px; line-height:1.8;">绪绪，今天的学习日报没有顺利生成。系统已经记录了失败信息，后续可以根据状态文件继续排查。</p></td></tr>
...
def build_failure_email_plain(report: dict | None, config: dict, target_date: str, failure_reason: str) -> str:
    site_base = config["site"]["base_url"].rstrip("/")
    lines = [
        config["email"]["sender_name"],
        target_date,
        "",
        "绪绪，今天的学习日报没有顺利生成。系统已经记录了失败信息，后续可以根据状态文件继续排查。",
        "",
...
def verify_inbox_delivery(config: dict, report: dict, target_date: str, timeout: int, interval: float, test_email: bool) -> dict:
    imap_cfg = require_imap_config(config)
    subject = build_subject(report, target_date, test_email)
    attempts = {"count": 0}
...
def main() -> int:
    ...
    try:
        ...
        test_email = args.test_email or args.dry_run or target_emails == [sender_email]
        ...
        if args.failure_notice:
            html = build_failure_email_html(report, config, target_date, args.failure_reason)
            plain = build_failure_email_plain(report, config, target_date, args.failure_reason)
        else:
            html = email_module.build_email_html(report, config, target_date, llm_content=llm_content)
            plain = email_module.build_email_plain(report, config, target_date, llm_content=llm_content)
        ...
        if args.dry_run:
            ...
            if args.verify_inbox:
                inbox_verification = {
                    "enabled": True,
                    **verify_inbox_delivery(config, report, target_date, args.verify_timeout, args.verify_interval, test_email),
                }
    ...
    write_agent_status(
        ...
        target_date,
        {
            ...
            "test_email": args.test_email or (target_emails == [(config.get("email") or {}).get("sender_email")]),
            ...
            "recipient_count": len(target_emails),
            ...
            "email": {
                ...
                "subject": (
                    build_failure_subject(target_date, args.test_email or args.dry_run or target_emails == [(config.get("email") or {}).get("sender_email")])
                    if args.failure_notice
                    else build_subject(report, target_date, args.test_email or args.dry_run or target_emails == [(config.get("email") or {}).get("sender_email")])
                )
                if report
                else None,
    ...
    if args.dry_run:
        ...
        print(f"[完成] 邮件已发送至 {len(target_emails)} 个收件人。")
    print(f"[状态] 已更新: {status_path}")
    return 0
